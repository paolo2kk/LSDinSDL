    ...
    if(result == NULL)
	{
        ...
        for (pugi::xml_node objectGroupNode = mapFileXML.child("map").child("objectgroup"); objectGroupNode != NULL; objectGroupNode = objectGroupNode.next_sibling("objectgroup")) {
            ...
            if (objectLayerName == "CollisionsDown") {
                colliderType = ColliderType::DOWN;
            }
            else if (objectLayerName == "CollisionsUp") {
                colliderType = ColliderType::UP;
            }
            else if (objectLayerName == "CollisionLeft") {
                colliderType = ColliderType::LEFT;
            }
            else if (objectLayerName == "CollisionsRight") {
                colliderType = ColliderType::RIGHT;
            }
        ...
        // Later you can create a function here to load and create the colliders from the map
            ...
            //for (const auto& mapLayer : mapData.layers) {
            //    //Check if the property Draw exist get the value, if it's true draw the lawyer
            //    if (mapLayer->properties.GetProperty("Collision") != NULL && mapLayer->properties.GetProperty("Collision")->value == true) {
            //        for (int i = 0; i < mapData.width; i++) {
            //            for (int j = 0; j < mapData.height; j++) {
            ...
            //                        //Get the screen coordinates from the tile coordinates
            //                        Vector2D mapCoord = MapToWorld(i, j);
            //                        Engine::GetInstance().physics->CreateRectangle(mapCoord.getX()+ mapData.tileWidth / 2, mapCoord.getY() + mapData.tileHeight / 2, mapData.tileWidth, mapData.tileHeight, bodyType::STATIC);
            //                    }
            //                }
            //            }
            //        }
